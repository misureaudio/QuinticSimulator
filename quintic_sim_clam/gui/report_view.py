...
class DocView(ttk.Frame):
    ...
    def __init__(self, master):
        ...
        self.pal = dict(PALETTES["light"])  # current color palette (theme.py)
        self._last_md: Optional[str] = None  # last rendered markdown
        self.canvas = tk.Canvas(self, highlightthickness=0, bg=self.pal["doc_bg"])
    ...
    def set_palette(self, pal: dict) -> None:
        """Switch the color palette; re-render the current document.

        Called by ``theme.apply`` on live theme switches. Colors only —
        the reflow machinery is untouched.
        """
        self.pal = dict(pal)
        self.canvas.configure(bg=self.pal["doc_bg"])
        if self._last_md is not None:
            self.render(self._last_md)
    ...
    def _styled_text(self, inlines, *, size: int, bg: str, bold: bool = False):
        ...
        txt.tag_configure("code", font=(CODE_FONT, size),
                          background=self.pal["code_bg"])
    ...
    def _window_bg(self) -> str:
        try:
            ...
            return self.pal["doc_bg"]

    def _add_paragraph(self, b: Paragraph) -> None:
        w = self._styled_text(b.inlines, size=10, bg=self.pal["doc_bg"])
        w.pack(fill="x", padx=8, pady=2)

    def _add_quote(self, b: Quote) -> None:
        outer = tk.Frame(self.inner, bg=self.pal["quote_border"], bd=0,
                         highlightthickness=0)
        inner = tk.Frame(outer, bg=self.pal["quote_bg"], bd=0,
                         highlightthickness=0)
        inner.pack(fill="both", expand=True, padx=(6, 8), pady=4)
        for blk in b.blocks:
            if isinstance(blk, Paragraph):
                self._styled_text(blk.inlines, size=10,
                                  bg=self.pal["quote_bg"]).pack(
                    fill="x", padx=2)
    ...
    def _add_code(self, b: CodeBlock) -> None:
        txt = tk.Text(
            ...
            wrap="none",
            font=(CODE_FONT, 9),
            background=self.pal["code_block_bg"],
    ...
    def _add_table(self, b: Table) -> None:
        ...
        for ci in range(ncol):
            ...
            # The LAST column stretches: with stretch=False everywhere the
            # vista Treeview shows its unthemed base background (white) in
            # the gap right of the fixed-width columns whenever the
            # document is wider than the table. Stretched, the last column
            # absorbs the slack and its tagged row background fills it.
            tree.column(cols[ci], width=self._col_width(col_texts),
                        minwidth=self.MIN_COL, anchor="w",
                        stretch=(ci == ncol - 1))
    ...
    def _expand_cell(self, tree: "ttk.Treeview", event) -> None:
        ...
        top.configure(bg=self.pal["window"])
