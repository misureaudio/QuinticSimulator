...
Phase 2 (ctk chrome): the widgets are customtkinter so the light/dark
switch is native. Key bindings must be attached to the *inner* tk.Entry
(``e._entry``) — synthetic and real key events do not reach the ctk
wrapper (verified against ctk 6.0.0).
"""
...
class CoeffPanel(ctk.CTkFrame):
    def __init__(self, master, *, on_run, on_stop, on_clear):
        super().__init__(master)
        # ctk 6.x has no CTkLabelFrame — title is a CTkLabel up top.
        ctk.CTkLabel(
            self,
            text="Polynomial    [c5] x^5 + [c4] x^4 + [c3] x^3 + "
                 "[c2] x^2 + [c1] x + [c0]",
            anchor="w",
        ...
        for i in range(6):
            ctk.CTkLabel(grid, text=LABELS[i], width=24, anchor="e").grid(
                row=0, column=2 * i, padx=2)
            e = ctk.CTkEntry(grid, width=60, justify="center")
            e.insert(0, DEFAULT)
            e.grid(row=0, column=2 * i + 1, padx=2)
            # ctk is canvas-based: key events only reach the inner entry
            inner = e._entry
            inner.bind("<KeyRelease>", lambda _e: self._update_preview())
            inner.bind("<Return>", lambda _e: self.run())
            self.entries.append(e)
            ctk.CTkLabel(grid, text=POWERS[i], anchor="w").grid(
                row=0, column=2 * i + 2, padx=2)

        self._prev = ctk.CTkLabel(
            self, text="", anchor="w",
        ...
        self.stop_btn = ctk.CTkButton(btns, text="Stop", command=on_stop,
                                      state="disabled")
        self.stop_btn.pack(side="left", padx=4)
        ctk.CTkButton(btns, text="Clear", command=self._clear).pack(
            side="left", padx=4)
