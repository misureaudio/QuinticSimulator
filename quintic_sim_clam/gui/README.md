...
```bash
.venv/Scripts/python -m quintic_sim.gui
```
...
`%APPDATA%\quintic_sim\config.json`, so you can point the GUI at any
interpreter or package copy, e.g.:

```
.venv/Scripts/python -m quintic_sim {poly} --sage
```

## Theme (light/dark)

A selector in the top bar switches the whole window live (chrome via
`ttk.Style` + `theme.apply`, document via `DocView.set_palette`) and
persists the choice as the `"theme"` key in the same config file. The CLI
shares the setting: `--set-theme dark` from a terminal makes the GUI start
dark (and vice versa), and `--theme` ANSI-colors a terminal run's output —
only on a tty, so the GUI's piped subprocess always receives plain
markdown (`FORCE_COLOR` in the parent environment is scrubbed from the
subprocess env).
...
| File | Responsibility |
|---|---|
| `__main__.py` | entry point (`python -m quintic_sim.gui`), DPI awareness |
| `config.py` | persistent command template + prefs (JSON) |
| `theme.py` | light/dark palettes + `apply()` (customtkinter optional, called lazily) |
...
- `customtkinter` is optional at runtime: `theme.apply()` calls
  `set_appearance_mode` only when it can be imported (Phase-2 chrome
  widgets use it natively; the Phase-1 ttk path works without it).
- Tests: `tests/test_gfx_*.py` (headless unit tests + widget tests +
  end-to-end through the real CLI), `tests/test_theme.py`,
  `tests/test_ansi.py`, `tests/test_cli_theme.py`.
