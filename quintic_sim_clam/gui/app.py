...
class QuinticApp:
    def __init__(self, root: tk.Tk):
        ...
        # ctk chrome (Phase 2): switches light/dark for free via
        # ctk.set_appearance_mode, driven by theme.apply() on every change.
        top = ctk.CTkFrame(root)
        top.pack(fill="x", padx=8, pady=(8, 2))
        ctk.CTkLabel(top, text="Command:").pack(side="left")
        self.cmd_var = tk.StringVar(value=self.cfg["command"])
        ctk.CTkEntry(top, textvariable=self.cmd_var).pack(
            side="left", fill="x", expand=True, padx=6)
        self.sage_var = tk.BooleanVar(value=bool(self.cfg["sage"]))
        ctk.CTkCheckBox(top, text="--sage", variable=self.sage_var).pack(
            side="left", padx=4)
        self.theme_var = tk.StringVar(
            value=gui_theme.normalize(self.cfg.get("theme")))
        self.theme_sel = ctk.CTkOptionMenu(
            top, values=list(gui_theme.THEME_NAMES),
            variable=self.theme_var, width=90)
        self.theme_sel.pack(side="left", padx=4)
        ctk.CTkLabel(
            top,
        ...
        # The constructor value does not fire the trace — apply once
        # explicitly so the window starts in the persisted theme.
        gui_theme.apply(self.root, self.theme_var.get(), self.doc)
    ...
    def _on_theme_change(self, *a) -> None:
        """Live retheme + persist (fired by the theme_var trace)."""
        gui_theme.apply(self.root, self.theme_var.get(), self.doc)
        self._save_cfg()
