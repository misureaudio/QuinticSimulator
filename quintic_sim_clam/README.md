...
window (and persisted to `%APPDATA%\quintic_sim\config.json`). A
**light/dark theme selector** in the top bar rethemes the window live
and persists to the same file the CLI writes via `--set-theme`.
...
| `--theme {light,dark}` | ANSI-color this run's terminal output (tty only; `FORCE_COLOR` overrides pipes, `NO_COLOR` disables) |
| `--set-theme {light,dark}` | persist the shared app theme (GUI + CLI) and use it for this run; with no polynomial, just set and exit |
